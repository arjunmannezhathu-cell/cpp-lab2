...
void runDemo() {
  ...
  // 2. Scenario: Placement of our battleship
  cout << "--- DEMO 1: Placing a Destroyer (B2-B4) ---" << endl;
  board->getOwnGrid().placeShip(Ship(GridPosition("B2"), GridPosition("B4")));
  view.print();

  // 3. Scenario: Rule Enforcement, here we try to place a ship that touches
  // the previous one
  ...
  // 4. Scenario: Combat, here we show hit on our own ship
  ...
  // 5. Scenario: Radar, here we show the opponent's radar
